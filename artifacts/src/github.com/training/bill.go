...
// logger
var chaincodeLogger = flogging.MustGetLogger("BillChaincode")
// 票据状态
const (
...
func (a *BillChaincode) issue(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	if len(args)!=1 {
		res := getRetString(1,"BillChaincode Invoke issue args!=1")
	...
	if err!=nil {
		res := getRetString(1,"BillChaincode Invoke issue unmarshal failed")
		return shim.Error(res)
	}
	// TODO 根据票号 查找是否票号已存在
	...
	if existbl {
		res := getRetString(1,"BillChaincode Invoke issue failed : the billNo has exist ")
		return shim.Error(res)
	}
	...
	if err != nil {
		res := getRetString(1,"BillChaincode Invoke issue failed :get time stamp failed ")
		return shim.Error(res)
	}
	...
	if err != nil {
		res := getRetString(1,"BillChaincode Invoke issue get bill list error")
		return shim.Error(res)
	}
	...
	for resultIterator.HasNext() {
		...
		if count >= 5 {
			res := getRetString(1,"BillChaincode Invoke issue The bill holder has more than 5 bills on the same day by the same type")
			return shim.Error(res)
		}
	}
	...
	if err != nil {
		res := getRetString(1,"BillChaincode Invoke issue put search table failed")
		return shim.Error(res)
	}
	...
	if !bl {
		res := getRetString(1,"BillChaincode Invoke issue put bill failed")
		return shim.Error(res)
	}
...
func (a *BillChaincode) endorse(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	if len(args)<3 {
		res := getRetString(1,"BillChaincode Invoke endorse args<3")
		return shim.Error(res)
	}
    // 根据票号取得票据
	bill, bl := a.getBill(stub, args[0])
	if !bl {
		res := getRetString(1,"BillChaincode Invoke endorse get bill error")
		return shim.Error(res)
	}

	if bill.HodrCmID == args[1] {
		res := getRetString(1,"BillChaincode Invoke endorse failed: Endorser should not be same with current Holder")
		return shim.Error(res)
	}
	...
	if err != nil {
		res := getRetString(1,"BillChaincode queryByBillNo GetHistoryForKey error")
		return shim.Error(res)
	}
	...
	for resultsIterator.HasNext() {
		...
		if err != nil {
			res := getRetString(1,"BillChaincode queryByBillNo resultsIterator.Next() error")
			return shim.Error(res)
		}
		...
		if hisBill.HodrCmID == args[1] {
			res := getRetString(1,"BillChaincode Invoke endorse failed: Endorser should not be the bill history holder")
			return shim.Error(res)
		}
	}
	...
	_, bl = a.putBill(stub, bill)
	if !bl {
		res := getRetString(1,"BillChaincode Invoke endorse put bill failed")
		return shim.Error(res)
	}
	...
	if err != nil {
		res := getRetString(1,"BillChaincode Invoke endorse put search table failed")
		return shim.Error(res)
	}
	stub.PutState(holderNameBillNoIndexKey, []byte{0x00})
...
func (a *BillChaincode) accept(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	if len(args)<3 {
		res := getRetString(1,"BillChaincode Invoke accept args<3")
		return shim.Error(res)
	}
    // 根据票号取得票据
	bill, bl := a.getBill(stub, args[0])
	if !bl {
		res := getRetString(1,"BillChaincode Invoke accept get bill error")
		return shim.Error(res)
	}
	...
	//holderNameBillNoIndexKey, err := stub.CreateCompositeKey(IndexName, []string{bill.HodrCmID, bill.BillInfoID})
	//if err != nil {
		//res := getRetString(1,"BillChaincode Invoke accept put search table failed")
		//return shim.Error(res)
	//}
	...
	_, bl = a.putBill(stub, bill)
	if !bl {
		res := getRetString(1,"BillChaincode Invoke accept put bill failed")
		return shim.Error(res)
	}
...
func (a *BillChaincode) reject(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	if len(args)<3 {
		res := getRetString(1,"BillChaincode Invoke reject args<3")
		return shim.Error(res)
	}
    // 根据票号取得票据
	bill, bl := a.getBill(stub, args[0])
	if !bl {
		res := getRetString(1,"BillChaincode Invoke reject get bill error")
		return shim.Error(res)
	}
	...
	if err != nil {
		res := getRetString(1,"BillChaincode Invoke reject put search table failed")
		return shim.Error(res)
	}
	stub.DelState(holderNameBillNoIndexKey)
	...
	_, bl = a.putBill(stub, bill)
	if !bl {
		res := getRetString(1,"BillChaincode Invoke reject put bill failed")
		return shim.Error(res)
	}
...
func (a *BillChaincode) queryMyBill(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	if len(args)!=1 {
		res := getRetString(1,"BillChaincode queryMyBill args!=1")
		return shim.Error(res)
	}
    // 以持票人ID从search表中批量查询所持有的票号
	billsIterator, err := stub.GetStateByPartialCompositeKey(IndexName, []string{args[0]})
	if err != nil {
		res := getRetString(1,"BillChaincode queryMyBill get bill list error")
		return shim.Error(res)
	}
	...
	for billsIterator.HasNext() {
		...
		if err != nil {
			res := getRetString(1,"BillChaincode queryMyBill SplitCompositeKey error")
			return shim.Error(res)
		}
        // 根据票号取得票据
		bill, bl := a.getBill(stub, compositeKeyParts[1])
		if !bl {
			res := getRetString(1,"BillChaincode queryMyBill get bill error")
			return shim.Error(res)
		}
	...
	if err != nil {
		res := getRetString(1,"BillChaincode Marshal queryMyBill billList error")
		return shim.Error(res)
	}
...
func (a *BillChaincode) queryMyWaitBill(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	if len(args)!=1 {
		res := getRetString(1,"BillChaincode queryMyWaitBill args!=1")
		return shim.Error(res)
	}
    // 以背书人ID从search表中批量查询所持有的票号
	billsIterator, err := stub.GetStateByPartialCompositeKey(IndexName, []string{args[0]})
	if err != nil {
		res := getRetString(1,"BillChaincode queryMyWaitBill GetStateByPartialCompositeKey error")
		return shim.Error(res)
	}
	defer billsIterator.Close()
	...
	for billsIterator.HasNext() {
		...
		_, compositeKeyParts, err := stub.SplitCompositeKey(kv.Key)
		if err != nil {
			res := getRetString(1,"BillChaincode queryMyWaitBill SplitCompositeKey error")
			return shim.Error(res)
		}
        // 根据票号取得票据
		bill, bl := a.getBill(stub, compositeKeyParts[1])
		if !bl {
			res := getRetString(1,"BillChaincode queryMyWaitBill get bill error")
			return shim.Error(res)
		}
	...
	b, err := json.Marshal(billList)
	if err != nil {
		res := getRetString(1,"BillChaincode Marshal queryMyWaitBill billList error")
		return shim.Error(res)
	}
	return shim.Success(b)
}
...
func (a *BillChaincode) queryByBillNo(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	if len(args)!=1 {
		res := getRetString(1,"BillChaincode queryByBillNo args!=1")
		return shim.Error(res)
	}
    // 取得该票据
	bill, bl := a.getBill(stub, args[0])
	if !bl {
		res := getRetString(1,"BillChaincode queryByBillNo get bill error")
		return shim.Error(res)
	}
	...
	if err != nil {
		res := getRetString(1,"BillChaincode Marshal queryByBillNo billList error")
		return shim.Error(res)
	}
	return shim.Success(b)
}
...
func (a *BillChaincode) Invoke(stub shim.ChaincodeStubInterface) pb.Response {
	...
	chaincodeLogger.Info("%s%s","BillChaincode function=",function)
	chaincodeLogger.Info("%s%s","BillChaincode args=",args)
    ...
    res := getRetString(1,"BillChaincode Unkown method!")
